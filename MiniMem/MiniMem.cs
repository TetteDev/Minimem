...
namespace MiniMem
{
	public class MiniMem
    {
	    ...
	    /// <summary>
	    /// Information about the current process we're attached to
	    /// </summary>
		...
		public static Signature[] FindPatternMultiple(string moduleName, Signature[] signatures, bool useParallel = true)
		{
			...
			ReadProcessMemory((int)AttachedProcess.ProcessHandle, pm.BaseAddress, buffer, buffer.Length, ref m_iNumberOfBytesRead);
		...
		public static Signature[] FindPatternMultiple(long startAddress, long endAddress, Signature[] signatures, bool useParallel = true)
		{
			...
			ReadProcessMemory((int)AttachedProcess.ProcessHandle, (IntPtr)startAddress, buff, buff.Length, ref m_iNumberOfBytesRead);
		...
		public static byte[] ReadBytes(long address, int byteCount)
		{
			...
			ReadProcessMemory((int)AttachedProcess.ProcessHandle, (IntPtr)address, buffer, byteCount, ref m_iNumberOfBytesRead);
			return buffer;
		}
		...
		public static string ReadString(long address, Encoding defaultEncoding = default(Encoding), int maxLength = 256, bool zeroTerminated = false)
		{
			...
			ReadProcessMemory((int)AttachedProcess.ProcessHandle, (IntPtr)address, buff, buff.Length, ref numBytesRead);
			return zeroTerminated ? defaultEncoding.GetString(buff).Split('\0')[0] : Encoding.UTF8.GetString(buff);
		}

		/// <summary>
		/// Read a value from a pointer + offsets
		/// </summary>
		...
		public static T ReadMultiLevelPointer<T>(long baseAddress, params int[] offsets) where T : struct
		{
			...
			{
				temp = ReadMemory<IntPtr>((long)temp + offsets[i]);
			}
			return ReadMemory<T>((long)temp + (long)offsets[offsets.Length - 1]);
		}
		...
		public static T ReadMemory<T>(long address) where T : struct
		{
			...
			ReadProcessMemory((int)AttachedProcess.ProcessHandle, (IntPtr)address, buffer, buffer.Length, ref m_iNumberOfBytesRead);
		...
		{
			if (AttachedProcess.ProcessHandle == IntPtr.Zero) throw new Exception("Memory module has not been attached to any process!");
			return WriteProcessMemory((int)AttachedProcess.ProcessHandle, (IntPtr)address, buffer, buffer.Length, out m_iNumberOfBytesWritten);
		}
		...
		public static bool WriteString(long address, string value, Encoding defaultEncoding = default(Encoding))
		{
			...
			return WriteProcessMemory((int)AttachedProcess.ProcessHandle, (IntPtr)address, memory, memory.Length, out var numBytesRead);
		}
		...
		public static bool WriteMultiLevelPointer<T>(long baseAddress, object value, params int[] offsets) where T : struct
		{
			...
			if (offsets.Length == 0)
			{
				return WriteMemory<T>(baseAddress, value);
			}
			else
			{
				var temp = ReadMemory<IntPtr>(baseAddress);
				...
				return WriteMemory<T>((long)temp + (long)offsets[offsets.Length - 1], value);
			}
		}
